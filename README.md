...
### 1. How to Play
* **Starting Capital**: Every user starts the game (Season 1) with **₹50,000** of virtual cash.
...
* **Total Exposure Cap**: A strict cap of **₹1,00,000** is enforced inclusive of the ₹50,000 starting capital. This means the maximum outstanding loan a user can borrow is **₹50,000**.
...
* **Net Worth Calculation**:
  $$\text{Net Worth} = \text{Cash} + \text{Market Value of Holdings} - \text{Loan Principal} - \text{Accrued Interest}$$
* **Going Bust**: If a user's net worth falls below **₹0 (or negative)**, they are flagged as bust and can trigger a fresh game restart.
* **Restarting Logic (Seasons)**:
  * Restarting liquidates all active holdings, settles outstanding debts, and issues a fresh **₹50,000** starting cash capital.
  * The game is bumped to the next **Season** (e.g., Season 2).
  * The final gain/loss of the closed season is added to the user's **Lifetime P&L** tracker. Prior losses or gains are preserved and displayed separately, so the user starts the new season with a clean sheet.
